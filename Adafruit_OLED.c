...
void writeCommand(unsigned char c)
{
    ...
    // Read dummy data to wait for transfer
...
void writeData(unsigned char c)
{
    ...
    // Read dummy data and wait for transfer
    MAP_SPIDataGet(GSPI_BASE, &ulDummy);
...
void Adafruit_Init(void)
{
     ...
     * OLED has been wired to GPIO28, pin 18 (P2.2).
     */

    // Reset the hardware
    GPIOPinWrite(GPIOA3_BASE, 0x10, 0x00);   // RESET low
    MAP_UtilsDelay(8000000);

    GPIOPinWrite(GPIOA3_BASE, 0x10, 0x10);   // RESET high
    MAP_UtilsDelay(8000000);
    ...
    writeCommand(SSD1351_CMD_DISPLAYON);       // turn on OLED
}
